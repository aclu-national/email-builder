const data = {
  address1: '303 E. 17th Ave., Suite 350',
  address2: 'Denver, CO 80203-1256',
  affiliation_name: 'ACLU of Colorado',
  donation_url: 'https://action.aclu.org/give/colorado-join-renew-today?initms_aff=co&initms_chan=eml&utm_medium=eml&initms=25XXXX_affcampaign_footerdonate_gradead_sail&utm_source=sail&utm_campaign=affcampaign&utm_content=25XXXX_civilliberties_footerdonate_gradead&af={sb_query_string_encrypted}&ms_aff=co&ms_chan=eml&ms=25XXXX_affcampaign_footerdonate_gradead_sail',
  logo: {
    custom: {
      size: '200',
      url: ''
    },
    default: {
      size: '150',
      url: 'https://media.sailthru.com/64d/1k9/8/f/689f77530c5c6.png'
    },
    eighty: {
      size: '200',
      url: ''
    },
    fifty: {
      size: '200',
      url: ''
    },
    newsletter: {
      size: '300',
      url: 'https://media.sailthru.com/64d/1k9/4/m/6807ca4fe2440.png'
    },
    ninety: {
      size: '200',
      url: ''
    },
    seventyFive: {
      size: '200',
      url: ''
    },
    sixty: {
      size: '200',
      url: ''
    },
    twenty: {
      size: '200',
      url: ''
    }
  },
  privacy_url: 'https://aclu-co.org/en/privacy-statement',
  signatures: {
    advo: {
      headshot: '',
      name: 'Sophia Mayott-Guerrero',
      pronouns: 'She, her, ella',
      title: 'Interim Director of Advocacy and Strategic Alliances'
    },
    comms: {
      headshot: '',
      name: 'Erica Tinsley',
      pronouns: 'She, her, hers',
      title: 'Interim Director of Communications'
    },
    custom1: {
      headshot: '',
      name: '',
      pronouns: '',
      title: ''
    },
    custom2: {
      headshot: '',
      name: '',
      pronouns: '',
      title: ''
    },
    custom3: {
      headshot: '',
      name: '',
      pronouns: '',
      title: ''
    },
    dev: {
      headshot: '',
      name: '',
      pronouns: '',
      title: 'Director of Philanthropy'
    },
    ed: {
      headshot: 'https://media.sailthru.com/64d/1k6/c/c/63979f5f3fa61.jpg',
      name: 'Deborah Richardson',
      pronouns: 'She, her, hers',
      title: 'Executive Director'
    },
    legal: {
      headshot: 'https://media.sailthru.com/64d/1k7/c/r/658c4eccf26df.jpg',
      name: 'Tim Macdonald',
      pronouns: 'He, him, his',
      title: 'Legal Director'
    },
    org: {
      headshot: '',
      name: '',
      pronouns: '',
      title: 'Organizing Director'
    }
  },
  social: {
    bluesky: '',
    facebook_url: '',
    instagram_url: '',
    threads_url: '',
    tiktok: '',
    x_url: '',
    youtube: ''
  },
  social: [
    {
      rank: 6,
      name: 'bluesky',
      url: 'https://bsky.app/profile/aclu.org',
    },
    {
      rank: 1,
      name: 'facebook',
      url: 'https://facebook.com/acluofcolorado',
    },
    {
      rank: 3,
      name: 'instagram',
      url: 'https://www.instagram.com/acluofcolorado',
    },
    {
      rank: 5,
      name: 'threads',
      url: 'https://www.threads.net/@aclu_nationwide',
    },
    {
      rank: 7,
      name: 'tiktok',
      url: 'https://www.tiktok.com/@aclu'
    },
    {
      rank: 2,
      name: 'x',
      url: 'https://x.com/acluofcolorado',
    },
    {
      rank: 4,
      name: 'youtube',
      url: 'https://www.youtube.com/aclu',
    }
  ],
  website_url: 'https://aclu-co.org'
};

module.exports = data;