...
if (!options.affiliation || !options.key || !options.value) {
  inquirer.prompt([
    {
      type: 'input',
      name: 'affiliation',
    ...
    const filePath = path.join(__dirname, '../_data/affiliations', `${answers.affiliation}.js`);
...
function updateFile(affiliation, key, value) {
  const filePath = path.join(__dirname, '../_data/affiliations', `${affiliation}.js`);
  ...
  import(filePath).then(content => {
    ...
    fs.writeFile(filePath, output, (err) => {
      if (err) {
        ...
        console.log(`Updated affiliation file at ${filePath}`);
      }
    });
  });
}